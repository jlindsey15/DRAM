...
log_filename = "translatedplain/copyglimpselog.csv"
load_file = "translatedplain/drawmodel40000.ckpt"
save_file = "translatedplain/copyglimpse_from_drawmodel40000_"
...
with tf.variable_scope("gX", reuse=None):
    ...
    for t in range(T):
        ...
        g_r, g_params=read(x,g_h_dec_prev)
...
fetches2=[]
fetches2.extend([reward, param_diff, g_train_op3])
...
for i in range(train_iters):
    ...
    reward_fetched, param_diff_fetched, _=results
    if i%100==0:
        print("iter=%d : Reward: %f" % (i, reward_fetched))
        print(param_diff_fetched)
        if i %1000==0:
            start_evaluate = time.clock()
            test_accuracy = 0#evaluate()
