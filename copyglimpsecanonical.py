...
log_filename = "translatedplain/copyglimpse_dm_can_decay_80k_log.csv"
load_file = "translatedplain/drawmodel_canonical80000.ckpt"
save_file = "translatedplain/copyglimpse_dm_can_decay_80k_"
...
for t in range(T):
    c_prev = tf.zeros((batch_size,28*28)) if t==0 else cs[t-1]
...
reconstruction_loss=tf.reduce_sum(binary_crossentropy(x_recons,x_recons),1) # reconstruction term
...
with tf.variable_scope("gX", reuse=None):
    ...
    for t in range(T):
        g_c_prev = tf.zeros((batch_size,28*28)) if t==0 else g_cs[t-1]
    ...
    g_reconstruction_loss=tf.reduce_sum(binary_crossentropy(g_x_recons,g_x_recons),1)
